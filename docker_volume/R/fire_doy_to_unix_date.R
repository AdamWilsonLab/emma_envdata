...
fire_doy_to_unix_date <- function(input_folder = "docker_volume/raw_data/fire_modis/",
  ...
  for(i in 1:length(input_files)){
    ...
    mask_i <- raster_i > 0
    raster_i <- raster::mask(x = raster_i, mask = mask_i,maskvalue = 0,updatevalue=NA)
    raster_i <- raster_i+year_i-1
    raster_i[!mask_i] <- 0
    
    #save output
    raster::writeRaster(x = raster_i,
                        filename = paste(output_folder,"/",date_i,".tif",sep = ""),overwrite=TRUE)
...
test <- raster("docker_volume/raw_data/fire_modis_integer_date/2000_11_01.tif")
plot(test)
test[2]
